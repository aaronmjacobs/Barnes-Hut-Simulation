...
//open the file for reading
void BasicModel::ReadFileObj(string filename) 
{
   string line;
   streampos linestart;
   ...
   if (infile.is_open())  
   {
      while (!infile.eof())
      {
         ...
         {
            Triangles.push_back(parseTriObj(line));
         }
      }
      infile.close();
   } 
...
BasicModel::Tri* BasicModel::parseTriObj(string line)
{
   Tri* t = new Tri();
